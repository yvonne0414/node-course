...
const StockDetails = () => {
  ...
  return (
    <div>
      <div className="m-6 text-right">共{stockDetails.length}筆</div>
      {stockDetails.map((detail) => {
        return (
          <div key={detail.date} className="bg-white bg-gray-50 p-6 rounded-lg shadow m-6">
            ...
            <h2 className="text-2xl font-bold mb-2 text-gray-800">成交金額：{detail.delta_price}</h2>
            ...
            <h2 className="text-2xl font-bold mb-2 text-gray-800">漲跌價差：{detail.transactions}</h2>
            ...
            <h2 className="text-2xl font-bold mb-2 text-gray-800">成交筆數：{detail.amount}</h2>
          </div>
        );
      })}
    </div>
